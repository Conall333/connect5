1. You will need npm and node.js installed to run the game, the game is run in the command line
2. the game can be run on the localhost , there is also a server running on 178.62.213.163
3. run npm install
4. start a local server using change the host in myConfig.json, then npm run runServer in the command line
5. start a client, type "npm run runClient" in the command line


Assumpitions:
A player will disconnect by typing "exit" into the number field or by using ctrl c
Players must connect to the server within 3 minutes of each other or they will be timed out
a move is checked to see if it is valid oin the client side